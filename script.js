    ...
    modules: [
      {
        ...
        description:
          "Core concepts, Multi-tenant architecture, and Data Model.",
        units: [
          {
            id: "admin-m1-u1",
            ...
            quiz: [
              {
                ...
                options: ["Profiles", "OWD", "Page Layouts"],
                correctAnswer: 1,
              },
            ],
          },
            ...
            qa: [
              {
                ...
                a: "1. System Validation, 2. Before Triggers, 3. Custom Validation, 4. Save to DB, 5. After Triggers, 6. Assignment Rules, 7. Workflow/Flow, 8. Commit.",
              },
              {
                q: "Can you modify a record in an After Trigger?",
                a: "No, the record is read-only in context. You must perform a DML update.",
              },
              {
                q: "What happens if a DML error occurs?",
            ...
            quiz: [
              {
                ...
                correctAnswer: 1,
                explanation:
                  "Before Triggers run before Custom Validation rules (but after System Validation).",
              },
            ],
          },
          {
...
* **CPU Time**: 10,000ms (10 seconds)
            ...
            qa: [
              {
                q: "What is the SOQL query limit?",
                a: "100 queries for synchronous transactions.",
              },
              {
                q: "How do you bypass the 50k row limit?",
                a: "Use a SOQL For Loop or Batch Apex.",
              },
              {
                q: "What is the heap size limit?",
                a: "6MB for synchronous, 12MB for asynchronous.",
              },
            ],
            quiz: [],
          },
...
\`\`\`
            ...
            qa: [
              {
                q: "When should you call Apex?",
                a: "In connectedCallback() or via Wire/Getter.",
              },
              {
                q: "Can you change props in the child component?",
                a: "No, props are read-only (@api). Use events to ask parent to change them.",
              },
            ],
            quiz: [],
          },
...
const ProgressManager = {
  ...
  get: (roleId) =>
    JSON.parse(localStorage.getItem(ProgressManager.getKey(roleId))) || [],
  save: (roleId, unitId) => {
    const current = ProgressManager.get(roleId);
    if (!current.includes(unitId)) {
      current.push(unitId);
      localStorage.setItem(
        ProgressManager.getKey(roleId),
...
function renderDashboard(roleId) {
  ...
  if (!role)
    return (document.body.innerHTML =
      '<div class="container" style="margin-top:2rem"><h1>Role Not Found</h1><a href="index.html" class="btn btn-secondary">Go Home</a></div>');
  ...
  if (iconContainer) {
    ...
    iconContainer.innerHTML = `<i data-lucide="${
      icons[roleId] || "star"
  ...
  role.modules.forEach((mod, index) => {
    ...
    const completedCount = mod.units.filter((u) =>
      ProgressManager.isComplete(roleId, u.id)
    ).length;
    const percent =
    ...
    el.onclick = () =>
      (window.location.href = `module.html?role=${roleId}&module=${mod.id}`);
...
function loadUnit(roleId, moduleId, index) {
  ...
  document.getElementById("unit-counter").innerText = `Unit ${index + 1} / ${
    mod.units.length
  }`;
  ...
  completeBtn.innerHTML = `
        <button id="mark-complete-btn" class="btn ${
          isComplete ? "btn-secondary" : "btn-primary"
        }" style="width:100%" onclick="toggleComplete('${roleId}', '${
    unit.id
  }', this)">
            ${
              isComplete
                ? 'Completed <i data-lucide="check" width="16"></i>'
                : "Mark as Complete"
            }
        </button>
    `;
    ...
    document.getElementById("quiz-content").innerHTML = `
            ...
            <div class="flex flex-col gap-2">
                ${q.options
                  .map(
                    (o, i) =>
                      `<label style="padding:0.75rem; border:1px solid var(--border-subtle); border-radius:var(--radius-sm); display:block; cursor:pointer;" class="quiz-option"><input type="radio" name="q" value="${i}"> <span style="margin-left:0.5rem; color:var(--text-secondary)">${o}</span></label>`
                  )
                  .join("")}
            </div>
            <button class="btn btn-primary" style="margin-top:1rem" onclick="checkQuiz(${
              q.correctAnswer
  ...
  nextBtn.innerHTML =
    index === mod.units.length - 1
      ? 'Finish Module <i data-lucide="check" width="16"></i>'
...
window.toggleComplete = function (roleId, unitId, btn) {
  ...
  if (isComplete) {
    ...
    // Update sidebar
    const sidebarItem = [...document.querySelectorAll(".sidebar-item")].find(
      (el) =>
        el.onclick.toString().includes(unitId) ||
    ...
    if (document.querySelector(".sidebar-item.active"))
      document.querySelector(".sidebar-item.active").classList.add("completed");
  }
  lucide.createIcons();
};
